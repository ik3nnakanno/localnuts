...
if(isset($_POST['submit']) && isset($_POST['food_name']) && isset($_FILES['food-img']) && isset($_POST['price'])){
    ...
    $img_name = $_FILES['food-img']['name'];
     ...
     if(mysqli_num_rows($result) > 0)
     {
        ...
        header("Location: chinese.inc.php?error=Food name already exist!");
        exit();
     
     }

     if (empty($food_name)) {
        header("Location: chinese.inc.php?error=Food name is required");
            exit();
    }elseif (empty($price)) {
        header("Location: chinese.inc.php?error=Pricing is required");
            exit();
    }else {
     ...
     if ($error === 0) {
        if ($img_size > 513024){
            header("Location: chinese.inc.php?error=File too large  > 512kb");
            exit();
    }else{
        ...
        if (in_array($img_ex_lc, $allowed_exs)){
            ...
            header("Location: chinese.inc.php?success=Upload Successful"); 
        }else{
            header("Location: chinese.inc.php?error=You can't upload files of this type");
            exit();
        }

    }
     }else{
        header("Location: chinese.inc.php?error=unknown error occured!");
            exit();
     }
}
