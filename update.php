...
if(isset($_POST['submit'])){   
    ...
    $img_name = $_FILES['food_img']['name'];
     $img_size = $_FILES['food_img']['size'];
     $tmp_name = $_FILES['food_img']['tmp_name'];
     $error = $_FILES['food_img']['error'];
     ...
     if (empty($food_name)) {
        header("Location: update.php?error=Food name is required");
            exit();
    }elseif (empty($price)) {
        header("Location: update.php?error=Pricing is required");
            exit();
    }else {
     ...
     if ($error === 0) {
        if ($img_size > 513024){
            header("Location: update.php?error=File too large");
            exit();
    }else{
        ...
        if (in_array($img_ex_lc, $allowed_exs)){
            ...
            $query = "UPDATE food SET food_name='$food_name', price='$price', food_image='$new_img_name' category_id='$category_id' WHERE food_id='$food_id'";
                
            $result = mysqli_query($conn, $query) or die("Operation failed".MYSQLI_ERROR($conn)); 
            
            header("Location: update.php?success=Updated Successful"); 
        }else{
            
           header("Location: update.php?error=Error processing your request");
            exit();
        }

    }
     }else{
        header("Location: update.php?error=Incomplete Data!");
           exit();
    }
}
?>
